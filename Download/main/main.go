...
func main() {
	...
	var futuresDaily, futuresIntraday60, futuresIntraday30, futuresIntraday15, barchart, yahoo, investing, coinapi bool
	...
	flag.BoolVar(&futuresIntraday15, "futures-intraday-15", false, "download futures intraday data from Barchart")
	...
	if (futuresDaily || futuresIntraday60 || futuresIntraday30 || futuresIntraday15) &&
		(!input.ValidateStringWithRegex(pattern, futuresStart) || !input.ValidateStringWithRegex(pattern, futuresEnd)) {
		pretty.ColorPrintln(errorColor, "invalid futures range")
		return
	}
	...
	if coinapi && (!input.ValidateStringWithRegex(pattern, cryptoStart) || !input.ValidateStringWithRegex(pattern, cryptoEnd)) {
		pretty.ColorPrintln(errorColor, "invalid crypto range")
		return
	}
	...
	if !futuresDaily && !futuresIntraday60 && !futuresIntraday30 && !futuresIntraday15 && !barchart && !yahoo && !investing && !coinapi {
		futuresDaily = true
		futuresIntraday60 = true
		futuresIntraday30 = true
		futuresIntraday15 = true
		barchart = true
		yahoo = true
		//investing = true
	}
	...
	if futuresIntraday15 {
		b = append(b, "Barchart Futures Intraday 15 Minutes")
	}
	...
	if futuresDaily || futuresIntraday60 || futuresIntraday30 || futuresIntraday15 {
		...
		if futuresIntraday15 {
			o := operator.NewBarchartFuturesOperator(istart, iend).IntradayFifteenMinutes()
			if futuresSymbols != "" {
				o.SetCustomSymbols(symbols)
			}
